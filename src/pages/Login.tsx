      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl shadow-xl p-8 relative z-10 transition-colors duration-300">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-slate-800/50 border ${errors.email ? 'border-rose-500 ring-rose-500/20' : 'border-slate-200 dark:border-slate-700/60'} rounded-xl text-sm font-semibold text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-forest-500/20 focus:border-forest-500 transition-all`}
                {...register('email')}
              />
            </div>
            {errors.email && (
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 px-4 flex items-center justify-center space-x-2 rounded-xl text-sm font-bold text-white bg-forest-600 hover:bg-forest-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 disabled:text-slate-400 transition-all duration-200 cursor-pointer shadow-md shadow-forest-600/10 hover:scale-[1.01] active:scale-[0.99]"
          >
            {loading ? (
              <>
